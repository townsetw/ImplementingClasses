...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.totaldistance = self.totaldistance + self.distance

    def move_by(self, dx, dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves_made = self.moves_made + 1
        self.totaldistance = self.totaldistance + ((dx) ** 2 + (dy) ** 2) **\
                                                  0.5
    ...
    def get_distance_from_start(self):
        return ((self.initialx - self.x) ** 2 + (self.initialy - self.y) ** 2)\
               **\
               0.5
    ...
    def closer_to(self, p2, p3):
        if (((self.x - p2.x)**2 + (self.y - p2.y)**2)**0.5) > (((self.x -
                                                                      p3.x)**2 + (self.y - p3.y)**2)**0.5):
            return p3
        elif (((self.x - p2.x)**2 + (self.y - p2.y)**2)**0.5) == (((self.x -
                                                                      p3.x)**2 + (self.y - p3.y)**2)**0.5):
            return p2
        else:
            return p2
    ...
    def halfway_to(self,p2):
        Point = (((self.x + p2.x) / 2), ((self.y + p2.y)/2))
        return Point
